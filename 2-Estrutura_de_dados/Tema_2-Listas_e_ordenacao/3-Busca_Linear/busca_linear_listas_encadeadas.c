...
int buscaLinearLista(struct No* primeiro, int valor);
...
int main()
{
    ...
    // Faz uma busca linear na lista para encontrar o valor 20 e exibir sua posição
    buscaLinearLista(primeiro, 20);
...
// ==================================================================================
// ======== BUSCAR LINEAR EM LISTAS ENCADEADAS ======================================
//Descrição: Procura um 'valor' na lista ligada a partir do 'primeiro'
// Retorno: A posição (índice) da primeira ocorrência do valor, se encontrado.
// -1, se o valor não estiver na lista
int buscaLinearLista(struct No* primeiro, int valor) {
    // Um contador para sabermos a posição (índice 0, 1, 2, ...) 
    int pos = 0;
    // Nosso ponteiro "viajante"
    struct No* atual = primeiro;
    printf("\n--- Buscando o índice de valor %d na lista ---\n", valor);
    // Enquanto não chegarmos ao fim da lista (NULL), continuamos procurando ...
    while (atual != NULL) {
        // Verificamos se o dado do nó atual é o que procuramos
        if (atual->dado == valor) {
            printf("Valor %d encontrado no índice %d.\n", valor, pos);
            // Encontramos o valor ! Retornamos a posição (índice) onde ele está.
            return pos;
        }
        // Se não for pulamos para o próximo nó ...
        atual = atual->proximo;
        // ... e incrementamos nosso contador de posição (índice).
        pos++;
    }
    // Se o while acabar, significa que percorremos toda a lista e não encontramos o valor.
    return -1;
} 
// ==========================================================================================
...
void exibirListaCompleta(struct No *primeiro)
{
    ...
    // 2. Enquanto nosso ponteiro "vuajante" não chegar ao fim (NULL)
    while (atual != NULL)
    {
