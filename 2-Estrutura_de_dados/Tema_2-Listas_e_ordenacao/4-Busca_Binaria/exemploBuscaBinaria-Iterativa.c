...
// --- Função de inicialização da MOCHILA DO FREE FIRE ---
...
// 5 Busca Binária
void buscaBinariaIterativa(Item *mochila, const char *nome);

// 5.1 Verifica se a mochila está vazia
...
int main()
{
    ...
    do
    {
        ...
        printf("4. Busca Binária Iterativa de Item\n");
        printf("0. Sair\n");
        ...
        case 4:
            if (mochilaVazia(&mochila) == 0)
            {
                char nomeBuscaBinariaIterativa[TAM_MAX_NOME];
                printf("\n--- Busca Binária Iterativa de Itens ---\n");
                printf("Digite o nome do item a ser buscado: ");
                fgets(nomeBuscaBinariaIterativa, TAM_MAX_NOME, stdin);
                nomeBuscaBinariaIterativa[strcspn(nomeBuscaBinariaIterativa, "\n")] = 0; // Remove o \n do enter
                buscaBinariaIterativa(&mochila, nomeBuscaBinariaIterativa);
            }
            break;
        case 0:
            printf("\nSaindo...\n");
...
// =====================================================================================
// Descrição: Procura por um "valor" no nosso caso "nomeBusca" em um "vetor/mochila"
// ORDENADO de um tamanho específico.
// Pré_requisito: O "vetor/mochila" DEVE estar em ordem CRESCENTE.
// Retorno : 0 índice do valor/nomeBusca, se encontrado.
//           -1, se o valor não estiver no vetor/mochila.
void buscaBinariaIterativa(Item *mochila, const char *nomeBuscaBinariaIterativa)
{
    int inicio = 0;
    ...
    while (inicio <= fim)
    {
        // int meio = (inicio + fim) / 2; Forma comun, mas pode dar overflow com número muito grandes.
        ...
        if (strcmp(mochila->nome[meio], nomeBuscaBinariaIterativa) == 0)
        {
            // Retorna o nome e o índice aonde o valor/nomeBusca foi encontrado
            printf("\nItem \"%s\" encontrado na posicao %d.\n", mochila->nome[meio], meio);
            printf("\nPressione ENTER para continuar...");
            getchar();
            return;
        }
        // 2. Se o valor no meio for menor que o valor/nomeBusca buscado,
        // significa que nosso alvo só pode estar na metade direita (maior).
        else if (strcmp(mochila->nome[meio], nomeBuscaBinariaIterativa) < 0)
        {
            inicio = meio + 1; // Descartamos a metade da esquerda
        }
        // 3. Se o valor/nomeBusca no meio for maior que o valor/nomeBusca buscado,
        // significa que nosso alovo só pode estar na metade da esquerda (menor).
        else
        {
            fim = meio - 1; // Descartamos a metade da direita
        }
    }

    // Se o loop terminar, significa que "inicio" ultrapassou "fim"
    // e o valor não foi encontrado no vetor/mochila
    printf("\nItem \"%s\" nao encontrado na mochila.\n", nomeBuscaBinariaIterativa);
    printf("\nPressione ENTER para continuar...");
    getchar();
}
