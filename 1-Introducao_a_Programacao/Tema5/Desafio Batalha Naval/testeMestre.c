...
void inicializarTabuleiro(int tabuleiro[linhaTabuleiro][colunaTabuleiro])
{
...
int main()
{
    ...
    int navio1[3] = {3, 3, 3};
    int navio1_i = 2; // Posição da Linha = i.
    int navio1_j = 0; // Posição  da Coluna = j.

    // Coordenadas de posição Navio 2.
    int navio2[3] = {3, 3, 3};
    int navio2_i = 9; // Posição da Linha = i.
    ...
    if (navio1_j + 3 > colunaTabuleiro || navio2_j + 3 > linhaTabuleiro || navio3_j + 3 > linhaTabuleiro)
    {
        printf("Erro: Um ou mais navios estão fora dos limites do tabuleiro.\n");
        return 1;
    }

    // Verificação sobreposição antes de posicionar os navios e ataques.
    for (int i = 0; i < 3; i++)
    {
        if (tabuleiro[navio1_i][navio1_j + i] != 0 || tabuleiro[navio2_i - i][navio2_j] != 0 || tabuleiro[navio3_i - i][navio3_j] != 0)
        {
            printf("Erro: sobreposição dos navios.\n");
            return 1;
        }
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // Posiona o navio 2 na vertical de baixo para cima.
        tabuleiro[navio2_i - i][navio2_j] = navio2[i];
        // Posiona o navio 3 na vertical de baixo para cima.
        tabuleiro[navio3_i - i][navio3_j] = navio3[i];
    }
